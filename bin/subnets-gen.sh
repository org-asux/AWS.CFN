...
if [ $# -ge 2 ] && [[ "$2" == "public"  || "$2" == "private" || "$2" == "public+natgw" ]]; then
	Scope=$2
else
...
${ORGASUXHOME}/asux.js yaml batch @${AWSCFNHOME}/bin/AWSCFN-${CFNContext}-${Scope}-Create.ASUX-batch.txt -i /dev/null -o ${CFNfile}
echo "aws cloudformation create-stack --stack-name ${MyVPCStackPrefix}-subnets-${Scope}-${JobSetName}  --region ${AWSRegion} --profile \${AWSprofile} --template-body file://${CFNfile} " > ${CFNfile}.sh
